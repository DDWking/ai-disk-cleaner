...
public sealed class AppSettings
{
    ...
    /// <summary>
    /// 「按文件夹删除」页自动把认不出用途的文件夹认出来。**默认开。**
    ///
    /// 它确实会自己发请求 —— 这是对 v2.9「翻页/展开 = 0 次模型请求」那条冻结的**有意放开**。
    /// 四条约束保证它不会失控：只问**还没认出来的**（问过的不再问，不会反复烧钱）、
    /// 停下 600ms 才发一次（展开/重建的抖动过程中不发）、换代即取消；
    /// 而且这里可以整个关掉。按量计费的端点用户可以关掉它。
    /// </summary>
    public bool AiAutoClassify { get; set; } = true;
