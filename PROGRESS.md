...
| 文件夹整理（辅助入口） | 可用 | **首屏只显示一级（全部收起）**；后台材料化一、二级并**只做本地识别**（不发模型请求）；三级及更深不自动；行内只有**单项 AI**（对一个文件夹主动发起，只分析这一个）；没有批量/自动 AI 入口；点用途展开行内详情（这是什么 / 为什么这么判断 / 来源）；Windows、Program Files、ProgramData、用户目录、AppData、Local/Roaming、下载都有**本地结论且不进 AI** |
