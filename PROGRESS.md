...
| 主导航 | 可用 | **三页：文件夹整理 / 清理中心 / 卸载**，默认进「文件夹整理」（扩展名统计已整页移除）；页面切换用枚举而非数字索引 |
| **文件夹整理（新首页）** | 可用 | **首屏只显示一级（全部收起）**；后台把一、二级材料化并**自动识别**（入口内部直接子目录也算一级，不点入口也会被识别）；三级及更深**不自动调用 AI**，只在深层工作条点「识别本层文件夹」分析这一层的直接子目录；行里没有逐项识别按钮、右边也没有第二个展开按钮；点用途展开行内详情（这是什么 / 为什么这么判断 / 来源）；Windows、Program Files、ProgramData、用户目录、AppData、Local/Roaming、下载都有**本地结论且不进 AI** |
