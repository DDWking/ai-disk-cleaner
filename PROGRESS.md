...
| AI 清理建议 | 规则可用 | 右侧「清理」页：可清理 / 大文件 / 老文件 / 重复 / 空文件夹 / 失效快捷方式 / 超长路径 / 和上次比 |
| 卸载页 | 可用 | 右侧第三页，BCU UninstallTools 列已装软件并走官方卸载。残留扫描未做 |
...
2. ~~规则清理面板~~ 已做
3. ~~卸载页~~ 已做（列已装 + 官方卸载；残留 / Steam / 驱动后做）
4. Treemap（自绘）
4. 可选本地小模型：只解释已勾选项，不把整盘丢进 LLM
