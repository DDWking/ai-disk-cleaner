...
# 1) locate an SDK-capable dotnet (per-user install first, then PATH).
#
# NOTE (PS 5.1): with $ErrorActionPreference = 'Stop', redirecting a native command's
# stderr raises NativeCommandError and kills the script, so relax the preference and
# decide by exit code plus the real --list-sdks output.
function Find-Dotnet {
    ...
    throw 'No .NET SDK found (neither ~/.dotnet/dotnet.exe nor dotnet on PATH has an SDK).'
}
